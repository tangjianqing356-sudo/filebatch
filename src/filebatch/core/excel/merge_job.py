...
（原来在 core/table_job.py，Excel 功能变多之后挪到 core/excel/ 下按模块拆开，
core/table_job.py 现在是一层兼容转发，老的导入路径仍然可用。）
...
from .workbook import (  # noqa: F401  （对外保持原有名字可用）
    CANDIDATE_ENCODINGS,
    CSV_SUFFIXES,
    EXCEL_SUFFIXES,
    MAX_ROWS,
...
def read_table(path: Path) -> list[list[str]]:
    """读成原始二维列表。保留这个名字是为了不破坏既有调用方和测试。"""
    rows, _sheet = read_rows(path)
    return rows
